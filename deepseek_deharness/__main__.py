...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Print a multi-run summary (rollout + per-log entry count) over one or more append-only logs; exit 0 iff all are healthy else 1.",
    )
    parser.add_argument(
        "--detail",
        metavar="LOG",
        ...
        type=str,
        default=None,
        help="Print a multi-run detail (summary + per-log message count) over one or more append-only logs; exit 0 iff all are healthy else 1.",
    )
    ...
    if args.summary is not None:
        result = batch_summary(args.summary)
        print(f"runs={result['runs']}")
        print(f"all_healthy={'yes' if result['all_healthy'] else 'no'}")
        print(f"total_entries={result['total_entries']}")
        print(f"max_estimated_tokens={result['max_estimated_tokens']}")
        print(f"identical_all={'yes' if result['identical_all'] else 'no'}")
        print(f"tool_calls_total={result['tool_calls_total']}")
        for i, (healthy, value, est, has_fr, n_tc, fr_len, entries) in enumerate(
            zip(
                result["healthy_per_log"],
                result["final_responses"],
                result["estimated_tokens_per_log"],
            ...
            print(
                ...
                f"final_response={value if value is not None else '-'} estimated_tokens={est} "
                f"has_final_response={'yes' if has_fr else 'no'} tool_calls={n_tc} "
                f"final_response_len={fr_len} entries={entries}"
            )
        return 0 if result["all_healthy"] else 1

    if args.detail is not None:
        result = batch_detail(args.detail)
        print(f"runs={result['runs']}")
        print(f"all_healthy={'yes' if result['all_healthy'] else 'no'}")
        print(f"total_entries={result['total_entries']}")
        print(f"max_estimated_tokens={result['max_estimated_tokens']}")
        print(f"identical_all={'yes' if result['identical_all'] else 'no'}")
        print(f"tool_calls_total={result['tool_calls_total']}")
        for i, (healthy, value, est, has_fr, n_tc, fr_len, entries, msg_count) in enumerate(
            zip(
                result["healthy_per_log"],
                result["final_responses"],
                result["estimated_tokens_per_log"],
            ...
            print(
                ...
                f"final_response={value if value is not None else '-'} estimated_tokens={est} "
                f"has_final_response={'yes' if has_fr else 'no'} tool_calls={n_tc} "
                f"final_response_len={fr_len} entries={entries} message_count={msg_count}"
            )
        return 0 if result["all_healthy"] else 1
