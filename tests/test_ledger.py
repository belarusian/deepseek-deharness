"""Tests for the multi-run ledger (ledger.py).

Prove batch_ledger composes manifest.batch_manifest into one consistent view with
per-log tool-call count detail, reports health/identity/outcome/size/health-per-log/
presence-per-log/tool-calls-per-log correctly across a ledger, and never mutates any
input log.
"""
...
from deepseek_deharness.ledger import batch_ledger
...
def test_ledger_two_identical_healthy_logs(tmp_path):
    a = _run_to_log(tmp_path, "a.jsonl")
    b = tmp_path / "b.jsonl"
    b.write_text(a.read_text())

    result = batch_ledger([a, b])
...
def test_ledger_pair_with_one_corrupted_log(tmp_path):
    a = _run_to_log(tmp_path, "a.jsonl")
...
def test_ledger_never_mutates_any_log(tmp_path):
    a = _run_to_log(tmp_path, "a.jsonl")
    b = tmp_path / "b.jsonl"
    b.write_text(a.read_text())
...
def test_ledger_empty_path_list(tmp_path):
    result = batch_ledger([])
...
def test_ledger_single_log_no_tool_calls(tmp_path):
    # A single healthy log with no tool calls: tool_calls_per_log is [0].
    a = tmp_path / "a.jsonl"
    a.write_text(
        '{"step": {"turn": 1}, "messages": [{"role": "user", "content": "hi"}]}\n'
    )

    result = batch_ledger([a])

    assert result["runs"] == 1
    assert result["tool_calls_per_log"] == [0]
