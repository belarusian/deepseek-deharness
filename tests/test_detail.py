"""Tests for the multi-run detail (detail.py).

Prove batch_detail composes summary.batch_summary into one consistent view with per-log
message-count detail, reports health/identity/outcome/size/health-per-log/presence-per-log/
tool-calls-per-log/final-response-len-per-log/entries-per-log/message-count-per-log correctly
across a detail, and never mutates any input log.
"""
from __future__ import annotations

from deepseek_deharness.audit import audit_log
from deepseek_deharness.detail import batch_detail
...
def test_detail_two_identical_healthy_logs(tmp_path):
    a = _run_to_log(tmp_path, "a.jsonl")
    b = tmp_path / "b.jsonl"
    b.write_text(a.read_text())

    result = batch_detail([a, b])
    ...
    # message_count_per_log: [m, m] where m is one log's message count
    # (== summarize_log(a)["message_count"], > 0).
    one_msg = summarize_log(a)["message_count"]
    assert one_msg > 0
    assert result["message_count_per_log"] == [one_msg, one_msg]


def test_detail_pair_with_one_corrupted_log(tmp_path):
    a = _run_to_log(tmp_path, "a.jsonl")
    ...
    # message_count_per_log still has length 2; the corrupt side's count reflects only its
    # parseable last entry.
    assert len(result["message_count_per_log"]) == 2
    assert result["message_count_per_log"][0] == summarize_log(a)["message_count"]


def test_detail_never_mutates_any_log(tmp_path):
    a = _run_to_log(tmp_path, "a.jsonl")
    b = tmp_path / "b.jsonl"
    b.write_text(a.read_text())
...
def test_detail_empty_path_list(tmp_path):
    result = batch_detail([])
    ...
    assert result["message_count_per_log"] == []


def test_detail_single_log_two_messages(tmp_path):
    # A single healthy log with one entry carrying two messages: message_count_per_log is [2].
    a = tmp_path / "a.jsonl"
    a.write_text(
        '{"step": {"turn": 1}, "messages": '
        '[{"role": "user", "content": "hi"}, {"role": "assistant", "content": "hello"}]}\n'
    )

    result = batch_detail([a])

    assert result["runs"] == 1
    assert result["message_count_per_log"] == [2]
