"""Tests for the multi-run rollout (rollout.py).

Prove batch_rollout composes ledger.batch_ledger into one consistent view with per-log
final-response length detail, reports health/identity/outcome/size/health-per-log/
presence-per-log/tool-calls-per-log/final-response-len-per-log correctly across a
rollout, and never mutates any input log.
"""
from __future__ import annotations

from deepseek_deharness.audit import audit_log
from deepseek_deharness.inspect import summarize_log
from deepseek_deharness.rollout import batch_rollout
...
def test_rollout_two_identical_healthy_logs(tmp_path):
    a = _run_to_log(tmp_path, "a.jsonl")
    b = tmp_path / "b.jsonl"
    b.write_text(a.read_text())

    result = batch_rollout([a, b])
...
def test_rollout_pair_with_one_corrupted_log(tmp_path):
    a = _run_to_log(tmp_path, "a.jsonl")
...
def test_rollout_never_mutates_any_log(tmp_path):
    a = _run_to_log(tmp_path, "a.jsonl")
    b = tmp_path / "b.jsonl"
    b.write_text(a.read_text())
...
def test_rollout_empty_path_list(tmp_path):
...
def test_rollout_single_log_no_assistant_content(tmp_path):
    # A single healthy log with no assistant content: final_response_len_per_log is [0].
    a = tmp_path / "a.jsonl"
    a.write_text(
        '{"step": {"turn": 1}, "messages": [{"role": "user", "content": "hi"}]}\n'
    )

    result = batch_rollout([a])

    assert result["runs"] == 1
    assert result["final_response_len_per_log"] == [0]
